...
interface BulkUploadResult {
    ...
    results: {
        success: Array<{ row: number; supplier: any }>;
...
export function BulkSupplierUploadDialog({ open, onOpenChange, onSuccess }: Props) {
    const [file, setFile] = useState<File | null>(null);
    const [uploading, setUploading] = useState(false);
    const [result, setResult] = useState<BulkUploadResult | null>(null);

    const handleDownloadTemplate = () => {
        // Direct download to avoid browser blocking "insecure" blob downloads
        // and to let the browser handle the file save dialog naturally.
        window.location.href = '/api/inventory/suppliers/template/download';
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await fetch('/api/inventory/suppliers/bulk-upload', {
                method: 'POST',
                body: formData,
                headers: {
            ...
            if (data.summary.successful > 0) {
                toast.success(`Successfully imported ${data.summary.successful} suppliers`);
                onSuccess?.();
            }
                ...
                <div className="space-y-6">
                    ...
                    <Alert>
                        ...
                        <AlertDescription className="flex items-center justify-between">
                            <span>Download the Excel template, fill it with your supplier data, and upload it here.</span>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-4 gap-4">
                                ...
                                <div className="bg-green-500/10 rounded-lg p-4">
                                    ...
                                    <p className="text-2xl font-bold text-green-700 dark:text-green-400">{result.summary.successful}</p>
                                </div>
                                ...
                                <div className="bg-yellow-500/10 rounded-lg p-4">
                                    ...
                                    <p className="text-2xl font-bold text-yellow-700 dark:text-yellow-400">{result.summary.duplicates}</p>
                                </div>
                            </div>
                                            ...
                                            <tbody>
                                                {result.results.success.slice(0, 10).map((item) => (
                                                    <tr key={item.row} className="border-t">
                                                        ...
                                                        <td className="p-2">{item.supplier.email || '-'}</td>
                                                        <td className="p-2">{item.supplier.phone || '-'}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
